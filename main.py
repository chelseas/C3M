...
parser.add_argument("--lr_step", type=int, default=10, help="")
parser.add_argument(
...
model_W, model_Wbot, model_u_w1, W_func, u_func = get_model(
...
# x = torch.stack([torch.from_numpy(x[0]) for x in X_te[:10000]]).float()
# x[:, 2] = 0.0
# x[:, 3] = 1.0
# x = x.requires_grad_()
# xref = torch.stack([torch.from_numpy(x[1]) for x in X_te]).float()
# uref = torch.stack([torch.from_numpy(x[2]) for x in X_te]).float()

# N = 5
# mixing_ratios = torch.linspace(0.0, 1.0, N)
# fix, axs = plt.subplots(1, N)

# for i in range(N):
#     mixing = mixing_ratios[i]
#     u_func.set_mixing(mixing)

#     u = u_func(x, x - xref[0], uref[0])  # u: bs x m x 1 # TODO: x - xref
#     K = Jacobian(u, x)

#     axs[i].scatter(x[:, 0].detach(), x[:, 1].detach(), c=K[:, 0, 0].detach())
#     axs[i].set_title(f"K[0, 0] ({1 - mixing} tanh + {mixing} hardtanh)")
#     axs[i].set_xlabel("x")
#     axs[i].set_ylabel("y")
...
for epoch in range(args.epochs):
    # Start mixing in hardtanh after some initial training
    if epoch >= 20:
        mixing = min((epoch - 20) / 10, 1.0)
        print(f"** Mixing {1 - mixing} Tanh + {mixing} Hardtanh **")
        W_func.set_mixing(mixing)
        u_func.set_mixing(mixing)
        reset = 20

    adjust_learning_rate(optimizer, epoch - reset)
    loss, _, _, _ = trainval(
        X_tr,
        train=True,
        _lambda=args._lambda,
        acc=False,
        detach=True if epoch - reset < args.lr_step else False,
    )
