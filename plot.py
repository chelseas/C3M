...
import numpy as np
from np2pth import get_system_wrapper, get_controller_wrapper, get_metric_wrapper
...
metric = get_metric_wrapper(args.pretrained_W)
...
if __name__ == "__main__":
    ...
    for n_traj in range(args.nTraj):
        ...
        errors.append(
            [
                np.sqrt(((x - xs) ** 2).sum()) / initial_dist
                for x, xs in zip(x_closed[n_traj][:-1], xstar)
            ]
        )

        if args.plot_type == "2D":
            plt.plot(
                [x[args.plot_dims[0], 0] for x in x_closed[n_traj]],
            ...
            plt.plot(
                [x[args.plot_dims[0], 0] for x in x_closed[n_traj]],
                [x[args.plot_dims[1], 0] for x in x_closed[n_traj]],
                [x[args.plot_dims[2], 0] for x in x_closed[n_traj]],
                "g",
                label="closed-loop traj" if n_traj == 0 else None,
            )
            ...
            for i, plot_dim in enumerate(args.plot_dims):
                plt.plot(
                    t, [x[plot_dim, 0] for x in x_closed[n_traj]][:-1], color=colors[i]
                )
        ...
        elif args.plot_type == "metric":
            plt.plot(
                t, [M for M in metrics[n_traj]]
            )

    if args.plot_type == "2D":
        plt.plot(
            [x[args.plot_dims[0], 0] for x in xstar],
            [x[args.plot_dims[1], 0] for x in xstar],
            "k",
            label="Reference",
        )
        plt.plot(
            xstar_0[args.plot_dims[0]], xstar_0[args.plot_dims[1]], "ro", markersize=3.0
        )
        plt.xlabel("x")
        plt.ylabel("y")
    elif args.plot_type == "3D":
        plt.plot(
            [x[args.plot_dims[0], 0] for x in xstar],
            [x[args.plot_dims[1], 0] for x in xstar],
            [x[args.plot_dims[2], 0] for x in xstar],
            "k",
            label="Reference",
        )
        plt.plot(
            xstar_0[args.plot_dims[0]],
            xstar_0[args.plot_dims[1]],
            xstar_0[args.plot_dims[2]],
            "ro",
            markersize=3.0,
        )
        ax.set_xlabel("x")
