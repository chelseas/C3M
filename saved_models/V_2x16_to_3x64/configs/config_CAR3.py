...
X_MIN = np.array([-5., -5., -np.pi]).reshape(-1,1)
X_MAX = np.array([5., 5., np.pi]).reshape(-1,1)
...
# for sampling ref
X_INIT_MIN = np.array([-2., -2., -1.])
X_INIT_MAX = np.array([ 2.,  2.,  1.])

XE_INIT_MIN = np.array([-1.,]*3)
XE_INIT_MAX = np.array([ 1.,]*3)

time_bound = 6.
time_step = 0.03
t = np.arange(0, time_bound, time_step)

state_weights = np.array([1,1,0.1])
